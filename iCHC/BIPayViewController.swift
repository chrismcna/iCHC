...
class BIPayViewController: UIViewController {
    ...
    @IBAction func send(_ sender: Any) {
        ...
        let privatePrefix = ChaincoinPrefixes.privatePrefix
        let pubkeyPrefix = ChaincoinPrefixes.pubKeyPrefix
        let minimumAmount: Int64 = 547
        
        var prefix = pubkeyPrefix
        
        if type == .P2SH {
            prefix = ChaincoinPrefixes.scriptHashPrefix
        }
        ...
        if let pubkeyHash = addressStr.publicAddressToPubKeyHash(prefix) {
            ...
            if let amount = Double(amountStr) {
                ...
                let convertedInCHC = Int64(amount * 100000000)
                
                if convertedInCHC > minimumAmount {
                    let txConstructor = TransactionDBConstructor(privateKeyPrefix: privatePrefix, publicKeyPrefix: pubkeyPrefix, sendAmount: convertedInCHC, to: RIPEMD160HASH(pubkeyHash.hexStringToNSData().reversedData), type: type, fee: 90000)
